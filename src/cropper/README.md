...
- Dont Supports zooming(remove)
- Dont Supports rotating(remove)
...
compoment/cropper
├── index.less
├── index.js          (export Cropper Class)
├── change.js         (croper size or position change)
├── default.js        (default config)
├── event.js          (bind event)
├── preview.js        (cropper)
├── render.js         (render view)
├── templage.js       (croper html template)
└── util.js           (tool)
```
