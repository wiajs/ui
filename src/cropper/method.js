...
import {getSize, getSourceCanvas, isUndefined, normalizeDecimalNumber} from './util'
...
export default {
  ...
  getCanvasData() {
    ...
    if (this.ready) {
      $.forEach(['left', 'top', 'width', 'height', 'naturalWidth', 'naturalHeight'], n => {
        data[n] = canvasData[n]
      })
    }
  ...
  getCroppedCanvas(opt = {}) {
    ...
    let {x: initialX, y: initialY, width: initialWidth, height: initialHeight} = this.getData()
  ...
  setDragMode(mode) {
    ...
    if (this.ready && !this.disabled) {
      ...
      dragBox.toggleClass(CLASS_CROP, croppable).toggleClass(CLASS_MOVE, movable)

      if (!opt.cropBoxMovable) {
        // Sync drag mode to crop box when it is not movable
        face.data(DATA_ACTION, mode)
        face.toggleClass(CLASS_CROP, croppable).toggleClass(CLASS_MOVE, movable)
      }
    }
